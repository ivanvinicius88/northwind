...
//FUNCIONARIOS
		...
		function cadFun($id, $sobrenome, $nome, $titulo, $titulocortesia, $nascimento, $admissao, $endereco, $cidade, $regiao, $cep, $pais, $telefone, $extensao,$notas){
...
//FUNCIONÁRIOS TERRITÓRIOS 
			//---------------Cadastrar Funcionários Territórios
			function inserirFunTer($idfuncionario, $idterritorio) {
			...
			function listaFunTer() {
	  		 ...
	  		 //---------------Buscar Funcionários Territórios
	   		function lookingFunTer($id){
				$sql = "SELECT * FROM funcionarios_territorios WHERE IDFuncionario = '{$id}'";
				$resultado = mysqli_query($this->database->getConexao(),$sql);
				return mysqli_fetch_assoc($resultado);
			}
				
			
			//---------------Atualizar Funcionários Territórios
			function UpdtFunTer($idfuncionario, $idterritorio ){
				$sql = "UPDATE funcionario_territorio SET IDTerritorio='{$idterritorio}' WHERE IDFuncionario = {$idfuncionario}";
				return mysqli_query($this->database->getConexao(), $sql);
			}


			//---------------Remover Funcionários Territórios
			function remFunTer($id) {
				$sql = "DELETE FROM `funcionarios_territorios` WHERE `funcionarios_territorios`.`IDFuncionario` = {$id}";
				mysqli_query($this->database->getConexao(), $sql);
			}
